...
// MODIFICATION: Use role-based authorization
$is_authorized = false;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action === 'reply') {
        ...
        if ($user_id) {
            ...
            // --- THIS IS THE FIX ---
            // Now we insert the 'type' as 'custom'
            $notification_link = "#"; // Link for admin replies (can be changed)
            $notification_type = "custom";
            $sql_insert_notification = "INSERT INTO notifications (user_id, message, link, type) VALUES (?, ?, ?, ?)";
            
            if($stmt_insert = mysqli_prepare($link, $sql_insert_notification)){
                mysqli_stmt_bind_param($stmt_insert, "isss", $user_id, $reply_text, $notification_link, $notification_type);
            // --- END FIX ---
        ...
        if ($item_data) {
            ...
            try {
                // MODIFIED: Added action_by column
