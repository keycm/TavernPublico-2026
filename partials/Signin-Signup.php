...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('/register', { method: 'POST', body: formData }); // <-- FIXED
                if (!response.ok) {
    ...
    if (resendRegisterOtpLink) {
        resendRegisterOtpLink.addEventListener('click', async (e) => {
            ...
            try {
                const response = await fetch('/resend_otp', { method: 'POST', body: formData }); // <-- FIXED
                const data = await response.json();
    ...
    if (signInForm) {
        signInForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('/login', { method: 'POST', body: formData }); // <-- FIXED
                const data = await response.json();
                if (data.success) {
                    if (data.redirect) {
    ...
    if (otpForm) {
        otpForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('/verify_otp', { method: 'POST', body: formData }); // <-- FIXED
                const data = await response.json();
    ...
    if (forgotPasswordForm) {
        forgotPasswordForm.addEventListener('submit', async (e) => {
            ...
            const userEmail = formData.get('email');
            try {
                const response = await fetch('/forgot_password', { method: 'POST', body: formData }); // <-- FIXED
                const data = await response.json();
                if (data.success) {
    ...
    if (resetOtpForm) {
        resetOtpForm.addEventListener('submit', async (e) => {
            ...
            const userEmail = formData.get('email');
            try {
                const response = await fetch('/verify_reset_otp', { method: 'POST', body: formData }); // <-- FIXED
                const data = await response.json();
                if (data.success) {
                    clearInterval(resendTimer);
    ...
    if (setNewPasswordForm) {
        setNewPasswordForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('/update_password', { method: 'POST', body: formData }); // <-- FIXED
                const data = await response.json();
